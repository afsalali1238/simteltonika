  ...
  try {
    ...
    if (extended) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        o += 2;
        // Explicit bounds check: unlike every read above, `subarray` does NOT
        // throw when it runs past the end — it CLAMPS. So a truncated NX element
        // used to slip through here with a short value and a cursor already past
        // the buffer, and the failure surfaced later as a bare RangeError from
        // readAvlFrame's `numData2` read, outside this try, unlabelled. Raising a
        // RangeError here routes it through the same F5 relabelling as every
        // other truncation.
        if (len > buf.length - o) {
          throw new RangeError('nx element length runs past the end of the record');
        }
...
export function readAvlFrame(buf, { maxPacketBytes = DEFAULT_MAX_PACKET_BYTES } = {}) {
  ...
  // F9: the data field must end exactly here. Nothing previously asserted that
  // the declared length was fully consumed, so a CRC-valid frame could carry
  // arbitrary trailing bytes — and `raw_frames` stores the frame verbatim as the
  // sealed artefact a utilisation dispute is argued from (invariant 8). Padding
  // that no parser ever looked at does not belong in the evidence root.
  if (o + 1 !== dataField.length) {
    throw new Error(
      `trailing bytes after record count: ${dataField.length - (o + 1)} unconsumed`,
    );
  }
